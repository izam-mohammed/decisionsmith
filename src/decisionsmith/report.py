...
class Report:
    ...
    def __str__(self) -> str:
        ...
        if by:
            lines += ["", "accuracy by who labelled the rows (against agent or LLM labels it measures agreement):"]
            lines += ["  %-28s %s on %d decisions" % (k, _fmt(v["accuracy"]), v["decisions"]) for k, v in by.items()]
