...
| [`teacher_anthropic.py`](teacher_anthropic.py) | Claude through Semantic Kernel (ANTHROPIC_API_KEY; pip install "semantic-kernel[anthropic]"). |
| [`teacher_azure_openai.py`](teacher_azure_openai.py) | Azure OpenAI through Semantic Kernel (AZURE_OPENAI_API_KEY). |
| [`teacher_ollama.py`](teacher_ollama.py) | A local Ollama model through Semantic Kernel (ollama pull qwen3; pip install "semantic-kernel[ollama]"). |
| [`teacher_openai.py`](teacher_openai.py) | OpenAI through Semantic Kernel (OPENAI_API_KEY). |

## Run

```bash
pip install "decisionsmith[semantic-kernel,laya]"
pip install semantic-kernel[anthropic,ollama]
export AZURE_OPENAI_API_KEY=...
python examples/04-integrations/semantic-kernel/in_framework_filter.py
python examples/04-integrations/semantic-kernel/in_framework_plugin.py
python examples/04-integrations/semantic-kernel/teacher_anthropic.py
python examples/04-integrations/semantic-kernel/teacher_azure_openai.py
python examples/04-integrations/semantic-kernel/teacher_ollama.py
python examples/04-integrations/semantic-kernel/teacher_openai.py
```
