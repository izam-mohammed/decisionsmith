...
`Dataset.map_batches(Decide(model))` for tasks, or `map_batches(Decide, fn_constructor_args=(build,), concurrency=n)` for an actor pool that builds the model or harness once per actor.
...
| file | what it shows |
|---|---|
| [`in_framework_actors.py`](in_framework_actors.py) | An actor pool: each actor builds the harness once (`fn_constructor_args=(build,)`) and decides many batches. |
| [`in_framework_tasks.py`](in_framework_tasks.py) | `Dataset.map_batches(Decide(model))`: decision columns for a Ray Dataset, one batch per task. |

## Run

```bash
uv add "decisionsmith[ray,laya]"
uv run python examples/04-integrations/ray/in_framework_actors.py
uv run python examples/04-integrations/ray/in_framework_tasks.py
```
